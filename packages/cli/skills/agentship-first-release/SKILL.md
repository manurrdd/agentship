...
the whole console itinerary for the stores the manifest declares, in order, with the number
of steps and fields in each. `action:"get"` on one id returns its console URL, its breadcrumb,
its ordered instructions, its fields with the values Agentship proposes, and the date the
instructions were last checked against the console.
...
2. **Credentials** — `agentship_configure_auth` per store.
...
4. **Write the ids back** — Apple assigns an App Store Connect app id. Put it in
   `.agentship/agentship.yaml` under `stores.apple.appId`; nothing that talks to Apple works
   until it is there. Google uses the package name, which is already in the manifest.
