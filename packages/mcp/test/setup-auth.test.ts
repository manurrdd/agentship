...
describe('configure_auth with a file path', () => {
  ...
  it('reads the .p8 from disk, warns about loose permissions, and never echoes the key', async () => {
  ...
  it('reports a Google credential as unverifiable — not rejected — without an app to test against', async () => {
  ...
  it('verifies Google against the package name once the session has a project', async () => {
    harness = await createMcpHarness({ stores: ['google'] });
